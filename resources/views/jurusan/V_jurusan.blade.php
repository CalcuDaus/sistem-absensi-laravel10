    ...
    <section class="container container-content position-relative">
        ...
        <div class="row w-75">
            <div class="col custom-card shadow-card overflow-x-auto position-relative">
                <div class="mt-2">
                    <a href="/admin/jurusan/tambah" class="btn btn-success mb-3"><i class="fa-solid fa-plus bold"></i>
                        Tambah</a>
                    <table id="example" class="display hover" style="width: 100%">
                        ...
                        <tbody>
                            <?php $i = 1; ?>
                            @forelse ($dt_jurusan as $s)
                                <tr>
                                    <td>{{ $i++ }}</td>
                                    <td>{{ $s['jurusan'] }}</td>
                                    <td>
                                        <form id="deleteForm" onsubmit="return confirm('Apakah Anda Yakin ?');"
                                            action="{{ route('hapus.jurusan', $s['id']) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                            <a href="{{ route('url.edit.jurusan', $s['id']) }}" title="Edit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="2">Data Jurusan Tidak Tersedia</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
